...
- A `NoteCard` is collapsed by default (`expanded` state, per-card, starts `false`) — only `entry.title` shows (single line, ellipsis-truncated via `.note-title`), since note bodies tend to be long and a fully-expanded list makes it hard to scan more than one or two at a time. The body `<pre>` only renders when expanded; an "Expand"/"Collapse" button in `.command-actions` toggles it, and the title line itself is also clickable to the same effect. `entry.body` itself is plain preformatted text — no markdown rendering (that was tried and reverted: CommonMark collapses consecutive non-blank lines with no separating blank line into one paragraph, silently destroying line breaks in exactly the dense, indentation-structured notes this app is for).
