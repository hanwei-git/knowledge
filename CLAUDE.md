...
- The note capture textarea's **Format** button runs `formatNoteBody()` (`organizer.ts`) — a one-click *layout* cleanup, not a markdown formatter: flush-left alignment for plain paragraph lines, ragged list indentation snapped to a consistent 2-spaces-per-level nesting (based on each line's indent relative to the list line before it), and a single blank line inserted as a section break between a list block and the paragraph text before/after it (any other run of blank lines collapsed to one). It mutates `rawBody` in place in the textarea — no auto-run, no live-as-you-type formatting.
