...
- A `NoteCard` is collapsed by default (`expanded` state, per-card, starts `false`), since note bodies tend to be long and a fully-expanded list makes it hard to scan more than one or two at a time. There's no separate title/summary line — the `.note-body` box itself is the toggle: collapsed it shows literally `"..."`, expanded it shows `entry.body`; clicking it flips `expanded`, and its `title` attribute (native browser tooltip) reads "Show detail" collapsed / "Collapse" expanded. `entry.body` itself is plain preformatted text — no markdown rendering (that was tried and reverted: CommonMark collapses consecutive non-blank lines with no separating blank line into one paragraph, silently destroying line breaks in exactly the dense, indentation-structured notes this app is for).
